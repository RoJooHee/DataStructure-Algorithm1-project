...
public class DonggukGameSoft_2 {
	...
	static int nth_element(int[] data, int s, int e, int k) {
		int p = partition1(data, s, e); //data[s..e]를 피봇값에 의해서 분리한다.
	...
	public static void main(String[] args) {
		...
		for(int k=1; k<=3; k++) {
			...
			double q30 = (double)(n*0.7); //전체에서 상위 30% 숫자
			int num30 = nth_element(data2, 0, n-1+1, (int)(q30)+1+1); //30%해당 값.  원래30%위치+현재 앞에 p_avg2이 포함되어 있으므로 +1
			int p_30=quicksort1(data2, p_avg2+1, n-1+1, num30); //p_avg는 원래값 아니므로 그 다음부터 n까지. 30%해당 값을 pivot으로 정렬했을 때의 pivot 인덱스
			
			double q20 = (double)(n*0.8); //전체에서 상위 20% 숫자
			int num20 = nth_element(data2, 0, n-1+2, (int)(q20)+1+2); //20%해당 값.  원래20%위치+p_avg2+p_30
			int p_20=quicksort1(data2, p_30+1, n-1+2, num20); //20%해당 값을 pivot으로 정렬했을 때의 pivot 인덱스
			
			double q10 = (double)(n*0.9); //전체에서 상위 10% 숫자
			int num10 = nth_element(data2, 0, n-1+3, (int)(q10)+1+3); //10%해당.  원래10%위치+p_avg2+p_30+p_20
			int p_10=quicksort1(data2, p_20+1, n-1+3, num10); //10%해당 값을 pivot으로 정렬했을 때의 pivot 인덱스
			
			for(int b=0; b<=n+3; b++) {
				if (b==p_avg2 || b==p_30-1 || b==p_20-2 || b==p_10-3) { //아래에서 삭제할수록 하나씩 인덱스 앞당겨지므로
					for (int c = b+1; c <= n+3; c++) { //pivot은 임의로 설정한 값이므로 하나씩 앞당기며 그 값 삭제
						data2[c-1] = data2[c]; }
				}
			}
